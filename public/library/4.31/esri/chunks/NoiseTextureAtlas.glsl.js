...
}`),c.fragment.code.add(a.glsl`vec3 fade(vec3 t) {
return (t * t * t) * (t * (t * 6.0 - 15.0) + 10.0);
  ...
  `));c.fragment.uniforms.add(new m.Float2PassUniform("weatherTile",p=>p.weatherTile)).code.add(a.glsl`
  ...
  `);g.mode===e.NoiseTextureRenderMode.Full?c.fragment.main.add(a.glsl`
      float padWidth = 1.0;
      ...
      `):c.fragment.main.add(a.glsl`
      vec2 mapUV = ${a.glsl.float(b.weatherTileSize)} * (gl_FragCoord.xy / ${a.glsl.float(b.atlasSize)});
      float map = abs(gradientNoise(mapUV));
      map = remap(map, 0.25 * (1.0 - worley(8.0 * mapUV)), 1.0, 0.0, 1.0);
      fragColor = vec4(map);
    `);return c}class h extends a.NoParameters{constructor(){super(...arguments);this.weatherTile=k.fromValues(0,0)}}const q=Object.freeze(Object.defineProperty({__proto__:null,NoiseTextureAtlasPassParameters:h,build:f},Symbol.toStringTag,{value:"Module"}));d.NoiseTextureAtlas=q;d.NoiseTextureAtlasPassParameters=h;d.build=f});
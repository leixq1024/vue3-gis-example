...
vec4 color = vec4(0.0);`);if(e.heightManifoldEnabled){d.uniforms.add(new r.Float2PassUniform("angleCutoff",b=>u(b)),new t.Float4PassUniform("heightPlane",(b,f)=>D(b.heightManifoldTarget,b.renderCoordsHelper.worldUpAtPosition(b.heightManifoldTarget,k),f.camera.viewMatrix)));const a=e.spherical?g.glsl`normalize(globalOrigin - pos)`:g.glsl`heightPlane.xyz`;d.main.add(g.glsl`
      ...
      color = max(color, heightManifoldColor * heightManifoldAlpha);`)}e.pointDistanceEnabled&&(d.uniforms.add(new r.Float2PassUniform("angleCutoff",a=>u(a)),new t.Float4PassUniform("pointDistanceSphere",(a,b)=>{c.transformMat4(B.getCenter(x),a.pointDistanceOrigin,b.camera.viewMatrix);x[3]=c.distance(a.pointDistanceOrigin,a.pointDistanceTarget);return x})),d.main.add(g.glsl`float pointDistanceSphereDistance = sphereDistancePixels(pointDistanceSphere, pos);
vec4 pointDistanceSphereColor = laserlineProfile(pointDistanceSphereDistance);
float pointDistanceSphereAlpha = 1.0 - smoothstep(angleCutoff.x, angleCutoff.y, abs(dot(normal, normalize(pos - pointDistanceSphere.xyz))));
color = max(color, pointDistanceSphereColor * pointDistanceSphereAlpha);`));e.lineVerticalPlaneEnabled&&(d.uniforms.add(new r.Float2PassUniform("angleCutoff",a=>u(a)),new t.Float4PassUniform("lineVerticalPlane",(a,b)=>{var f=I.pointAt(a.lineVerticalPlaneSegment,.5,k);f=a.renderCoordsHelper.worldUpAtPosition(f,M);const N=c.normalize(n,a.lineVerticalPlaneSegment.vector);f=c.cross(k,f,N);c.normalize(f,f);return D(a.lineVerticalPlaneSegment.origin,f,b.camera.viewMatrix)}),new l.Float3PassUniform("lineVerticalStart",
(a,b)=>{const f=c.copy(k,a.lineVerticalPlaneSegment.origin);a.renderCoordsHelper.setAltitude(f,0);return c.transformMat4(f,f,b.camera.viewMatrix)}),new l.Float3PassUniform("lineVerticalEnd",(a,b)=>{const f=c.add(k,a.lineVerticalPlaneSegment.origin,a.lineVerticalPlaneSegment.vector);a.renderCoordsHelper.setAltitude(f,0);return c.transformMat4(f,f,b.camera.viewMatrix)})),d.main.add(g.glsl`if (pointIsWithinLine(pos, lineVerticalStart, lineVerticalEnd)) {
...
a=>a.intersectsLineRadius)),d.main.add(g.glsl`if (pointIsWithinLine(pos, intersectsLineStart, intersectsLineEnd)) {
float intersectsLineDistance = lineDistancePixels(intersectsLineStart, intersectsLineDirection, intersectsLineRadius, pos);
vec4 intersectsLineColor = laserlineProfile(intersectsLineDistance);
float intersectsLineAlpha = 1.0 - smoothstep(angleCutoff.x, angleCutoff.y, 1.0 - abs(dot(normal, intersectsLineDirection)));
color = max(color, intersectsLineColor * intersectsLineAlpha);
}`));d.main.add(g.glsl`fragColor = laserlineOutput(color * depthDiscontinuityAlpha);`);return m}function u(e){return G.set(O,Math.cos(e.angleCutoff),Math.cos(Math.max(0,e.angleCutoff-y.deg2rad(2))))}function D(e,m,d){c.transformMat4(E,e,d);c.copy(h,m);h[3]=0;z.transformMat4(h,h,d);return A.fromPositionAndNormal(E,h,P)}const F=y.deg2rad(6),O=w.create(),k=q.create(),h=H.create(),M=q.create(),n=q.create(),E=q.create(),P=A.create(),x=B.create();w=Object.freeze(Object.defineProperty({__proto__:null,build:C,
defaultAngleCutoff:F},Symbol.toStringTag,{value:"Module"}));v.Laserlines=w;v.build=C;v.defaultAngleCutoff=F});
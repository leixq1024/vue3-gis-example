...
return stretchedVal;
}`);const a=d.applyColormap?e.glsl`fragColor = applyBackgroundBlend(colormap(vec4(grayVal, grayVal, grayVal, currentPixel.a), !u_useGamma));`:e.glsl`fragColor = applyBackgroundBlend(vec4(grayVal, grayVal, grayVal, currentPixel.a));`;b.fragment.main.add(e.glsl`
    ...
    vec4 currentPixel = getPixel(pixelLocation);
    ${d.stretchType===k.RasterColorizerStretchType.Noop?e.glsl`fragColor = applyBackgroundBlend(currentPixel);`:e.glsl`
...
fragColor = applyBackgroundBlend(vec4(hillshade, hillshade, hillshade, alpha));`;b.main.add(e.glsl`
      vec2 pixelLocation = getPixelLocation(uv);
      if (isOutside(pixelLocation)) {
        fragColor = applyBackgroundBlend(vec4(0.0, 0.0, 0.0, 0.0));
        return;
